...
const DEFAULT_CONFIG = {
  ...
  pixelsPerSec: 11, rowHeight: 38, visibleCycles: 2,
};
...
function DragHandle() {
  return (
    <span style={{ cursor: "grab", color: "#B0ADA8", fontSize: 13, lineHeight: 1, userSelect: "none", paddingRight: 2 }}
...
function DepArrow({ dep, activities, rows, PPS, RH, CD }) {
  ...
  return (
    <g>
      ...
      {dep.nextCycle && (
        <text x={cx} y={Math.min(y1,y2)-4} textAnchor="middle"
...
function SortBar({ onSort }) {
  return (
    <div style={{ display:"flex", gap:4, alignItems:"center" }}>
      <span style={{ fontSize:8, color:"#7A7870" }}>ordenar:</span>
      <IconBtn small onClick={() => onSort("start")}>por inicio</IconBtn>
      <IconBtn small onClick={() => onSort("row_start")}>por fila+inicio</IconBtn>
    </div>
  );
}
...
export default function GanttCiclico() {
  ...
  // ── Drag-to-reorder for rows, activities, deps ───────────────────────────
  ...
  // ── Timeline drag (move/resize bars) ─────────────────────────────────────
  ...
  const sortActivities = (key) => {
    setActivities(prev => [...prev].sort((a, b) => {
      if (key === "start")     return a.start - b.start;
      if (key === "row_start") return a.row !== b.row ? a.row - b.row : a.start - b.start;
      return 0;
    }));
  };
  const autoSortDeps = () => {
  ...
  // ── Drag-row style helper ─────────────────────────────────────────────────
  const drStyle = (drag, i) => ({
    opacity: drag.dragIdx === i ? 0.4 : 1,
    background: drag.overIdx === i ? "#D4F4EB" : undefined,
    outline: drag.overIdx === i ? "2px solid #0E8A6A" : undefined,
    borderRadius: 4,
    ...
    <div style={{ fontFamily:"'DM Mono','Fira Mono','Courier New',monospace", background:"#F5F3EE", minHeight:"100vh" }}>
      ...
      <div style={{ background:"#1C1B18", padding:"9px 16px", display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
        <div>
          ...
          <div style={{ color:"#7A7870", fontSize:8.5, marginTop:1 }}>ciclos industriales · dependencias · multi-ciclo · reordenar · guardar/cargar</div>
        </div>
        <div style={{ flex:1 }} />
        {violations.length > 0 && (
        ...
        <button onClick={handleSave}
          style={{ background:"#3B5278", color:"#fff", border:"none", borderRadius:4, padding:"4px 10px", fontSize:9.5, fontWeight:700, cursor:"pointer" }}>⬇ Guardar</button>
        <button onClick={() => fileInputRef.current?.click()}
          style={{ background:"#5A5855", color:"#fff", border:"none", borderRadius:4, padding:"4px 10px", fontSize:9.5, fontWeight:700, cursor:"pointer" }}>⬆ Cargar</button>
        <input ref={fileInputRef} type="file" accept=".json" onChange={handleLoad} style={{ display:"none" }} />
        <button onClick={addActivity}
          style={{ background:"#5B4FD8", color:"#fff", border:"none", borderRadius:4, padding:"4px 10px", fontSize:9.5, fontWeight:700, cursor:"pointer" }}>+ Actividad</button>
        <button onClick={addDep}
          style={{ background:"#0E8A6A", color:"#fff", border:"none", borderRadius:4, padding:"4px 10px", fontSize:9.5, fontWeight:700, cursor:"pointer" }}>+ Dep.</button>
      </div>
        ...
        <div style={{ padding:"12px 16px" }}>
          <div style={{ display:"flex", alignItems:"baseline", gap:12, marginBottom:7 }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:5, fontSize:8.5, color:"#5A5855" }}>
              ciclos:
              {[1,2,3].map(n => (
                <IconBtn key={n} active={NC===n} small onClick={() => setConfig(p=>({...p,visibleCycles:n}))}>{n}</IconBtn>
              ))}
            </div>
          </div>
              ...
              {ticks.map(t => (
                <g key={t}>
                  <line x1={LW+t*PPS} y1={38} x2={LW+t*PPS} y2={totalH}
                    stroke={t%(step*2)===0?"#C4C2BA":"#EBEBEA"} strokeWidth={t%(step*2)===0?1:0.5} />
              ...
              {Array.from({length:NC+1},(_,ci)=>ci).map(ci => (
                <g key={ci}>
                  <line x1={LW+ci*CD*PPS} y1={34} x2={LW+ci*CD*PPS} y2={totalH}
                    stroke="#C04B2A" strokeWidth={ci===0||ci===NC?2:1.5} strokeDasharray={ci===0||ci===NC?"5 3":"3 3"} />
                  {ci < NC && (
                    <text x={LW+ci*CD*PPS+4} y={28}
                      style={{fontSize:8,fill:"#C04B2A",fontWeight:700,fontFamily:"inherit"}}>ciclo {ci+1}</text>
                  )}
                </g>
              ))}
              {rows.map((row,ri) => {
              ...
              <g transform={`translate(${LW},0)`}>
                {Array.from({length:NC-1},(_,ci)=>ci+1).map(ci =>
                  activities.map(act => {
                    ...
                    return <ActivityBar key={`${act.id}-c${ci}`} act={act} rowY={48+ri*ROWH}
                ...
                {activities.map(act => {
                  const ri=rows.findIndex(r=>r.id===act.row); if(ri<0) return null;
                  return <ActivityBar key={act.id} act={act} rowY={48+ri*ROWH}
                ...
                {depResults.map(dep => (
                  <DepArrow key={dep.id} dep={dep} activities={activities} rows={rows} PPS={PPS} RH={RH} CD={CD} />
                ))}
              </g>
          ...
          <div style={{ display:"flex", gap:12, marginTop:9, flexWrap:"wrap", alignItems:"center" }}>
            ...
            {maxWrap>0 && <div style={{fontSize:8.5,color:"#C04B2A",display:"flex",alignItems:"center",gap:4}}><div style={{width:14,borderTop:"2px dashed #C04B2A"}} />↩ wrap-around</div>}
            <div style={{fontSize:8.5,color:"#C04B2A",display:"flex",alignItems:"center",gap:4}}><span style={{fontSize:9}}>+1</span> dep. ciclo siguiente</div>
      ...
      {tab === "activities" && (
        <div style={{ padding:"12px 16px" }}>
          <div style={{ display:"flex", gap:7, marginBottom:10, alignItems:"center", flexWrap:"wrap" }}>
            ...
            <SortBar onSort={sortActivities} />
          </div>

          {/* ── FILAS ── */}
          <div style={{ fontSize:8.5, fontWeight:700, color:"#5A5855", letterSpacing:1, marginBottom:5 }}>
            FILAS <span style={{fontSize:7.5,fontWeight:400,color:"#9B9890"}}>— arrastrar ⠿ para reordenar</span>
          </div>
          <div style={{ display:"flex", flexDirection:"column", gap:3, marginBottom:14 }}>
            {rows.map((row, i) => (
              <div key={row.id}
                draggable
                onDragStart={() => rowDrag.onDragStart(i)}
                onDragOver={e => { e.preventDefault(); rowDrag.onDragOver(i); }}
                onDrop={() => rowDrag.onDrop(i)}
                onDragEnd={rowDrag.onDragEnd}
                style={{ display:"flex", gap:5, alignItems:"center", background:"#EEECEA", borderRadius:4, padding:"3px 7px", ...drStyle(rowDrag, i) }}>
                <DragHandle />
          ...
          {/* ── ACTIVIDADES ── */}
          <div style={{ fontSize:8.5, fontWeight:700, color:"#5A5855", letterSpacing:1, marginBottom:5 }}>
            ACTIVIDADES <span style={{fontSize:7.5,fontWeight:400,color:"#9B9890"}}>— arrastrar ⠿ para reordenar</span>
          </div>
          <div style={{ display:"flex", flexDirection:"column", gap:3 }}>
            {activities.map((act, i) => {
              const col=COLOR_PALETTE[act.color]||COLOR_PALETTE.gray, open=selected===act.id;
              const end=act.start+act.duration, wraps=end>CD;
              return (
                <div key={act.id}
                  draggable={!open}
                  onDragStart={()=>actDrag.onDragStart(i)}
                  onDragOver={e=>{e.preventDefault();actDrag.onDragOver(i);}}
                  onDrop={()=>actDrag.onDrop(i)}
                  onDragEnd={actDrag.onDragEnd}
                    ...
                    <div style={{padding:"7px 10px",background:"#fff",display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
                      ...
                      <label style={{display:"flex",flexDirection:"column",gap:2}}>
                        ...
                        <input type="number" min={1} value={act.duration}
                          ...
                          style={{padding:"2px 5px",borderRadius:3,fontSize:9.5,fontFamily:"inherit",border:`1px solid ${wraps?"#FCA5A5":"#D3D1C7"}`}} />
                      </label>
                      <label style={{display:"flex",flexDirection:"column",gap:2}}>
      ...
      {tab === "deps" && (
        <div style={{ padding:"12px 16px" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8, gap:8, flexWrap:"wrap" }}>
            <div style={{ fontSize:8.5, fontWeight:700, color:"#5A5855", letterSpacing:1 }}>
              DEPENDENCIAS <span style={{fontSize:7.5,fontWeight:400,color:"#9B9890"}}>— arrastrar ⠿ · auto-ordenar por origen</span>
            </div>
            <div style={{ display:"flex", gap:6 }}>
              <button onClick={autoSortDeps}
                style={{background:"#5A5855",color:"#fff",border:"none",borderRadius:4,padding:"3px 9px",fontSize:9.5,fontWeight:700,cursor:"pointer"}}>
                ↕ Auto-ordenar
              </button>
              <button onClick={addDep}
                style={{background:"#0E8A6A",color:"#fff",border:"none",borderRadius:4,padding:"3px 9px",fontSize:9.5,fontWeight:700,cursor:"pointer"}}>
                + Nueva
              </button>
            </div>
          </div>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:5 }}>
            {depResults.map((dep, i) => {
              ...
              return (
                <div key={dep.id}
                  draggable
                  onDragStart={()=>depDrag.onDragStart(i)}
                  onDragOver={e=>{e.preventDefault();depDrag.onDragOver(i);}}
                  onDrop={()=>depDrag.onDrop(i)}
                  onDragEnd={depDrag.onDragEnd}
                  style={{
                  ...
                  <div style={{display:"flex",gap:5,alignItems:"center",flexWrap:"wrap"}}>
                    ...
                    <label style={{display:"flex",gap:3,alignItems:"center",fontSize:8.5,cursor:"pointer",
                      ...
                      <input type="checkbox" checked={!!dep.nextCycle}
                        onChange={e=>updDep(dep.id,"nextCycle",e.target.checked)} style={{margin:0}} />
      ...
      {tab === "legend" && (
        <div style={{ padding:"12px 16px", maxWidth:520 }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
            {COLOR_KEYS.map(k => {
              const col=COLOR_PALETTE[k], count=activities.filter(a=>a.color===k).length;
              return (
      ...
      {tab === "config" && (
        <div style={{ padding:"12px 16px", display:"flex", flexDirection:"column", gap:9, maxWidth:400 }}>
          {[
            ["title","Título","text"],["subtitle","Subtítulo","text"],
          ...
          <div style={{background:"#EEECEA",borderRadius:5,padding:"8px 10px",fontSize:8.5,color:"#5A5855"}}>
            <strong>Guardar / Cargar:</strong> botones del header. El JSON incluye todo el modelo — actividades, filas, dependencias, configuración y nombres de colores.
          </div>
        </div>
      )}
    </div>
